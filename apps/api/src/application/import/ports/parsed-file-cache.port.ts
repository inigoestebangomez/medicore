...
//
// Per spec §3 / AD-6 the raw uploaded buffer is never persisted to the DB —
// only the explicitly-confirmed importedData JSONB is (BR-IMP-002). But the
// cleaning (Stage 3), matching (Stage 4) and finalize (Stage 5) steps all
// need the full parsed rows, which can be large. This cache bridges the
// synchronous upload request and the background finalize job without storing
// PII in a durable table. Entries are purged on COMPLETED, FAILED or revert.
//
